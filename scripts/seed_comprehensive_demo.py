...
_p0_email = _p0.email if _p0 else "?"
_p1_email = _p1.email if _p1 else "?"
print()
print("═"*64)
print("  Virtual Quiz States — QA State Map")
print("═"*64)
print()
print("  ── Belépési adatok ──────────────────────────────────────")
print("   admin:    admin@lfa.com           /  Admin123!")
print(f"   player1:  {_p0_email:<35s}  /  Demo#1234")
print(f"   player2:  {_p1_email:<35s}  /  Demo#1234")
print()
print(f"  [DONE — REWARDS_DISTRIBUTED]")
print(f"   t_id={_vq_done['t_id']}  sess={_vq_done['sess_id']}  quiz={_vq_done['quiz_id']}")
print(f"   → {_p0_email:<40s} PASSED ✅  score=100%  xp=50")
print(f"   → {_p1_email:<40s} FAILED ❌  score=33%   xp=0")
print(f"   Expected UI: Completed event + rankings visible")
print(f"   URL:  http://localhost:8000/events/{_vq_done['t_id']}")
print()
print(f"  [ACTIVE — IN_PROGRESS, quiz kitölthető]")
print(f"   t_id={_vq_active['t_id']}  sess={_vq_active['sess_id']}  quiz={_vq_active['quiz_id']}")
print(f"   → {_p0_email:<40s} NO ATTEMPT — 'Take Quiz' gomb látható")
print(f"   → {_p1_email:<40s} NO ATTEMPT — 'Take Quiz' gomb látható")
...
print()
print(f"  [INTERRUPTED — megkezdett, el nem küldött attempt]")
print(f"   t_id={_vq_interrupted['t_id']}  sess={_vq_interrupted['sess_id']}  quiz={_vq_interrupted['quiz_id']}")
print(f"   → {_p0_email:<40s} IN PROGRESS — 2/3 answered, completed_at=NULL")
print(f"   → {_p1_email:<40s} NO ATTEMPT — 'Take Quiz' gomb")
print(f"   URL:  http://localhost:8000/events/{_vq_interrupted['t_id']}")
print()
print(f"  [FAILED — kitöltve, score < passing (33% < 60%)]")
print(f"   t_id={_vq_failed['t_id']}  sess={_vq_failed['sess_id']}  quiz={_vq_failed['quiz_id']}")
print(f"   → {_p0_email:<40s} FAILED ❌  score=33%  attempt 1/3  → 'Retry Quiz'")
print(f"   → {_p1_email:<40s} NO ATTEMPT — 'Take Quiz' gomb")
