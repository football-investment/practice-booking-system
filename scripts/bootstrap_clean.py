...
What it creates (skips existing rows):
  ...
  8. Multi-campus infra  6 campuses, 18 pitches, 4 extra instructors
  9. InvitationCode      5 stock (unused) + 1 per student (used, INV-BOOTSTRAP-{id})
                         + CreditTransaction audit trail per student
"""
import json
import os
import subprocess
...
def _seed_invitation_codes(db, admin: User) -> dict:
    """
    Step 9: Idempotent invitation code seeding.

    Creates:
      A) 5 stock (unused) codes — for manual registration testing
      B) 1 historical (used) code per bootstrap student (format: INV-BOOTSTRAP-{user_id})
      C) 1 CreditTransaction per student — initial allocation audit trail

    Idempotency:
      - InvitationCode: unique-check on .code column
      - CreditTransaction: unique-check on idempotency_key
    """
    ...
    _STOCK = [
        {"code": "INV-DEMO-OPEN01",  "invited_name": "Open Registration 1",  "bonus_credits": 100},
        {"code": "INV-DEMO-OPEN02",  "invited_name": "Open Registration 2",  "bonus_credits": 100},
        {"code": "INV-DEMO-OPEN03",  "invited_name": "Open Registration 3",  "bonus_credits": 250},
        {"code": "INV-DEMO-PARTNER", "invited_name": "Partner Demo Code",    "bonus_credits": 500},
        {"code": "INV-DEMO-TEST01",  "invited_name": "Manual Test Code",     "bonus_credits":  50},
    ]

    now_utc = datetime.now(timezone.utc)

    # ── A) Stock codes (unused) ────────────────────────────────────────────────
    created_stock = skipped_stock = 0
    for sc in _STOCK:
        exists = db.query(InvitationCode).filter(InvitationCode.code == sc["code"]).first()
        if not exists:
            db.add(InvitationCode(
                code=sc["code"],
                invited_name=sc["invited_name"],
                bonus_credits=sc["bonus_credits"],
                is_used=False,
                created_by_admin_id=admin.id,
                notes="Seeded by bootstrap_clean.py — for manual registration testing",
            ))
            created_stock += 1
        else:
            skipped_stock += 1
    db.flush()
    _ok(f"Stock codes (unused): {created_stock} new, {skipped_stock} already existed")

    # ── B+C) Historical used codes + CreditTransaction per bootstrap student ───
    boot_club = db.query(Club).filter(Club.code == _CLUB_CODE).first()
    if not boot_club:
        _skip("Bootstrap club not found — no historical codes created")
        return {"total": 0, "used": 0, "unused": 0, "total_credits": 0}

    students = (
        db.query(User)
        .join(TeamMember, TeamMember.user_id == User.id)
        .join(Team, Team.id == TeamMember.team_id)
        .filter(Team.club_id == boot_club.id, User.role == UserRole.STUDENT)
        .distinct()
        .all()
    )

    created_hist = skipped_hist = 0
    created_tx = skipped_tx = 0

    for user in students:
        # B) Historical invitation code — INV-BOOTSTRAP-{user_id}
        code_str = f"INV-BOOTSTRAP-{user.id}"
        exists = db.query(InvitationCode).filter(InvitationCode.code == code_str).first()
        if not exists:
            db.add(InvitationCode(
                code=code_str,
                invited_name=user.name,
                bonus_credits=user.credit_balance,
                is_used=True,
                used_by_user_id=user.id,
                used_at=now_utc,
                created_by_admin_id=admin.id,
                notes="Historical bootstrap code — auto-generated by bootstrap_clean.py",
            ))
            created_hist += 1
        else:
            skipped_hist += 1

        # C) CreditTransaction audit trail for initial credit allocation
        ikey = f"bootstrap-credit-{user.id}"
        tx_exists = db.query(CreditTransaction).filter(
            CreditTransaction.idempotency_key == ikey
        ).first()
        if not tx_exists:
            db.add(CreditTransaction(
                ...
                description="Bootstrap initial credit allocation",
    ...
    used_codes   = db.query(InvitationCode).filter(InvitationCode.is_used.is_(True)).count()
    unused_codes = total_codes - used_codes
    total_cred   = db.query(func.sum(InvitationCode.bonus_credits)).filter(
        InvitationCode.is_used.is_(True)
    ).scalar() or 0
    return {"total": total_codes, "used": used_codes, "unused": unused_codes, "total_credits": total_cred}
...
def run():
    ...
    try:
        ...
        # Step 9: Invitation Codes
        _step(9, "Invitation Codes (5 stock unused + 1 per bootstrap student, used)")
        inv_stats = _seed_invitation_codes(db, admin)
        db.commit()
        ...
        print(f"  InvitationCodes: {inv_stats['total']} total "
              f"({inv_stats['used']} used / {inv_stats['unused']} unused), "
              f"{inv_stats['total_credits']:,} credits mapped")
        print()
