#!/usr/bin/env python3
"""
Test Suite for gancuju_attendance table
...
def test_01_table_exists():
    """Test that gancuju_attendance table exists with correct columns"""
    ...
    try:
        ...
        cur.execute("""
            SELECT EXISTS (
                ...
                WHERE table_name = 'gancuju_attendance'
            )
        """)
...
def test_02_insert_attendance():
    ...
    try:
        ...
        cur.execute("SELECT id FROM gancuju_enrollments WHERE is_active = TRUE LIMIT 1")
        ...
        cur.execute("""
            INSERT INTO gancuju_attendance (
                enrollment_id, session_id, status, checked_in_at
            )
...
def test_03_unique_constraint():
    ...
    try:
        ...
        try:
            cur.execute("""
                INSERT INTO gancuju_attendance (enrollment_id, session_id, status)
                VALUES (%s, %s, 'ABSENT')
            """, (enrollment_id, session_id))
            conn.commit()
...
def test_04_check_constraints():
    ...
    try:
        ...
        try:
            cur.execute("""
                INSERT INTO gancuju_attendance (enrollment_id, session_id, status, checked_in_at)
...
def test_05_cascade_delete():
    ...
    try:
        ...
        # Delete enrollment
        cur.execute("DELETE FROM gancuju_enrollments WHERE id = %s", (test_enrollment_id,))
        conn.commit()

        # Check attendance was cascade deleted
        cur.execute("SELECT COUNT(*) FROM gancuju_attendance WHERE id = %s", (attendance_id,))
...
def test_07_indexes():
    ...
    try:
        cur.execute("""
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'gancuju_attendance'
        """)
        indexes = [row[0] for row in cur.fetchall()]

        required_indexes = [
            'gancuju_attendance_pkey',
            'idx_gancuju_attendance_unique',
            'idx_gancuju_attendance_enrollment',
            'idx_gancuju_attendance_session',
            'idx_gancuju_attendance_status',
            'idx_gancuju_attendance_present'
        ]
...
if __name__ == '__main__':
    print("=" * 60)
    print("🧪 GANCUJU ATTENDANCE TABLE - TEST SUITE")
    print("=" * 60)

    # Run SQL file first
    sql_file = '/Users/lovas.zoltan/Seafile/Football Investment/Projects/Football Investment Internship/practice_booking_system/implementation/01_database_migration/10_create_gancuju_attendance.sql'
