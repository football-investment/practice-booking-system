#!/usr/bin/env python3
"""
Test Suite for gancuju_enrollments table
...
def test_01_table_exists():
    """Test that gancuju_enrollments table exists with correct columns"""
    ...
    try:
        ...
        cur.execute("""
            SELECT EXISTS (
                ...
                WHERE table_name = 'gancuju_enrollments'
            )
        """)
...
def test_02_insert_enrollment():
    ...
    try:
        ...
        cur.execute("SELECT id FROM gancuju_licenses WHERE is_active = TRUE LIMIT 1")
        license_row = cur.fetchone()
        if not license_row:
            # Create a license for testing
            cur.execute("""
                INSERT INTO gancuju_licenses (
                    user_id, current_level, sessions_attended,
                    competitions_entered, competitions_won, teaching_hours
                )
                VALUES (2, 3, 10, 5, 2, 0)
                RETURNING id
            """)
        ...
        # Insert enrollment
        cur.execute("""
            INSERT INTO gancuju_enrollments (
                license_id, semester_id,
                payment_verified, payment_proof_url, payment_reference_code
            )
            VALUES (%s, %s, TRUE, 'https://example.com/proof.jpg', 'GC123456')
...
def test_03_unique_active_constraint():
    ...
    try:
        ...
        try:
            cur.execute("""
                INSERT INTO gancuju_enrollments (license_id, semester_id, is_active)
                VALUES (%s, %s, TRUE)
            """, (license_id, semester_id))
            conn.commit()
...
def test_04_check_constraints():
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE is_active = TRUE
            AND id NOT IN (
                SELECT semester_id FROM gancuju_enrollments
        ...
        try:
            cur.execute("""
                INSERT INTO gancuju_enrollments (license_id, semester_id, payment_proof_url)
        ...
        try:
            cur.execute("""
                INSERT INTO gancuju_enrollments (license_id, semester_id, payment_reference_code)
                VALUES (%s, %s, 'AB')
            """, (license_id, semester_id))
            conn.commit()
...
def test_05_foreign_key_cascade():
    ...
    try:
        ...
        # Create enrollment
        cur.execute("""
            INSERT INTO gancuju_enrollments (license_id, semester_id)
            VALUES (%s, %s)
            RETURNING id
        ...
        # Delete license
        cur.execute("DELETE FROM gancuju_licenses WHERE id = %s", (test_license_id,))
        conn.commit()

        # Check enrollment was cascade deleted
        cur.execute("SELECT COUNT(*) FROM gancuju_enrollments WHERE id = %s", (enrollment_id,))
...
def test_07_indexes():
    ...
    try:
        cur.execute("""
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'gancuju_enrollments'
        """)
        indexes = [row[0] for row in cur.fetchall()]

        required_indexes = [
            'gancuju_enrollments_pkey',  # Primary key
            'idx_gancuju_enrollments_unique_active',
            'idx_gancuju_enrollments_license',
            'idx_gancuju_enrollments_semester',
            'idx_gancuju_enrollments_verified'
        ]
...
if __name__ == '__main__':
    print("=" * 60)
    print("🧪 GANCUJU ENROLLMENTS TABLE - TEST SUITE")
    print("=" * 60)

    # Run SQL file first
    sql_file = '/Users/lovas.zoltan/Seafile/Football Investment/Projects/Football Investment Internship/practice_booking_system/implementation/01_database_migration/06_create_gancuju_enrollments.sql'
