...
class TestParallelspecializationsSmoke:
    ...
    def test_get_semester_specialization_info_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"GET /api/v1/semester-info/{semester} should require auth or error: {response.status_code}"
        )
    ...
    def test_validate_specialization_addition_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"GET /api/v1/validate/{specialization} should require auth or error: {response.status_code}"
        )
