...
class TestSpecializationsSmoke:
    ...
    def test_get_level_info_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"GET /api/v1/level-info/{specialization_id}/{level} should require auth or error: {response.status_code}"
        )
    ...
    def test_get_specialization_levels_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"GET /api/v1/levels/{specialization_id} should require auth or error: {response.status_code}"
        )
    ...
    def test_update_progress_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"POST /api/v1/update-progress/{specialization_id} should require auth or error: {response.status_code}"
        )
