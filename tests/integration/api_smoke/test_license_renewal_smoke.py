...
class TestLicenserenewalSmoke:
    ...
    def test_get_expiring_licenses_auth_required(
        ...
        assert response.status_code in [401, 403], (
            f"GET /api/v1/expiring should require auth: {response.status_code}"
        )
    ...
    def test_get_license_status_happy_path(
        ...
        # Accept 200, 201, 404 (if resource doesn't exist in test DB)
        assert response.status_code in [200, 201, 404], (
            f"GET /api/v1/status/{license_id} failed: {response.status_code} "
            f"{response.text}"
        )
    ...
    def test_get_license_status_auth_required(
        ...
        # Should return 401 Unauthorized or 403 Forbidden
        assert response.status_code in [401, 403], (
            f"GET /api/v1/status/{license_id} should require auth: {response.status_code}"
        )
    ...
    def test_bulk_check_expirations_auth_required(
        ...
        # Should return 401 Unauthorized or 403 Forbidden
        assert response.status_code in [401, 403], (
            f"POST /api/v1/check-expirations should require auth: {response.status_code}"
        )
    ...
    def test_renew_license_auth_required(
        ...
        # Should return 401 Unauthorized or 403 Forbidden
        assert response.status_code in [401, 403], (
            f"POST /api/v1/renew should require auth: {response.status_code}"
        )
