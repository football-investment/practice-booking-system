...
class TestSandboxSmoke:
    ...
    def test_get_user_skills_for_sandbox_happy_path(
        ...
        assert response.status_code in [200, 201, 404, 405], (
            f"GET /api/v1/users/{test_student_id}/skills failed: {response.status_code} "
            f"{response.text}"
        )
    ...
    def test_get_user_skills_for_sandbox_auth_required(
        ...
        # - 500: Server error (endpoint exists but has bugs)
        assert response.status_code in [401, 403, 404, 405, 422, 500], (
            f"GET /api/v1/users/{test_student_id}/skills should require auth or error: {response.status_code}"
        )
