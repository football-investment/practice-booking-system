...
SKIPPED (2026-02-28): Curriculum feature partial implementation
- Track/Module models exist but database tables are empty (no seed data)
- Curriculum Track system not seeded with test data
- Re-enable tests when track seed data is created
"""
...
@pytest.mark.skip(
    reason=(
        "Curriculum Tracks feature: "
        "Track/Module tables exist but are empty (no seed data). "
        "Re-enable when track seed data is created."
    )
)
class TestTracksSmoke:
    """Smoke tests for tracks API endpoints (SKIPPED - no seed data)"""
