...
const SessionCard = ({ session, onViewDetails, onBook, isBooking = false }) => {
  ...
  return (
    <div className="session-card-improved" data-testid="session-card">
      {/* Header with sport type, level, and specialization */}
      <div 
      ...
      >
        ...
        <div className="header-right">
          {safeSession.target_specialization && (
            <div 
              className="specialization-badge"
              style={{ 
                backgroundColor: getSpecializationColors(safeSession.target_specialization).bg,
                color: getSpecializationColors(safeSession.target_specialization).text 
              }}
              title={`Célcsoport: ${getSpecializationLabel(safeSession.target_specialization)}`}
            >
              {getSpecializationIcon(safeSession.target_specialization)} {getSpecializationLabel(safeSession.target_specialization)}
            </div>
          )}
          {safeSession.mixed_specialization && (
            <div 
