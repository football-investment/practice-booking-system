...
const MilestoneTracker = ({ milestones = [], overallProgress = 0, compact = false, projectId, onRefresh }) => {
  ...
  const handleSubmitMilestone = async (milestoneId) => {
    if (!projectId || submitting[milestoneId]) return;
    
    setSubmitting(prev => ({ ...prev, [milestoneId]: true }));
    
    try {
      await apiService.submitMilestone(projectId, milestoneId);
      
      // Show success message (optional)
      if (window.showToast) {
        window.showToast('Mérföldkő sikeresen beküldve értékelésre!', 'success');
      }
      ...
      // Show error message
      if (window.showToast) {
        window.showToast('Hiba a mérföldkő beküldése során: ' + error.message, 'error');
      }
    } finally {
      setSubmitting(prev => ({ ...prev, [milestoneId]: false }));
    }
  };

  const canSubmitMilestone = (milestone) => {
          ...
          >
            ...
            <div className="milestone-content">
              ...
              <div className="milestone-details">
                ...
                {/* Submit button for eligible milestones */}
                {canSubmitMilestone(milestone) && (
                  <div className="milestone-actions">
                    <button 
                      className="milestone-submit-btn"
                      onClick={() => handleSubmitMilestone(milestone.milestone_id)}
                      disabled={submitting[milestone.milestone_id]}
                    >
                      {submitting[milestone.milestone_id] ? '⏳ Beküldés...' : '📤 Beküldés értékelésre'}
                    </button>
                  </div>
                )}
              </div>
