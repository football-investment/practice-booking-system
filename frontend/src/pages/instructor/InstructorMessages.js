...
const InstructorMessages = () => {
  ...
  const loadConversationMessages = useCallback(async (partnerId) => {
    try {
      ...
      // 🐛 DEBUG: Log message structure and ownership check (Instructor Messages)
      console.log('🔍 Instructor Message structure check:', {
        user: user,
  ...
  const loadConversationsQuietly = useCallback(async () => {
    try {
      ...
      console.error('Failed to quietly load conversations:', err);
    }
  }, []);
  ...
  if (loading) {
    return (
      <div className="instructor-messages">
  ...
  return (
    <div className={`instructor-messages chat-layout ${hasNewMessages ? 'has-new-messages' : ''}`}>
                    ...
                    <div className="conversation-info">
                      <div className="conversation-name">
                        ...
                        {conversation.unreadCount > 0 && (
                          <span className={`unread-badge ${hasNewMessages ? 'pulse' : ''}`}>{conversation.unreadCount}</span>
                        )}
                      </div>
