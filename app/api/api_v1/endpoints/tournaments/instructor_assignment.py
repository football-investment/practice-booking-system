...
def accept_instructor_assignment(
    ...
    **Raises:**
    ...
    # LicenseValidator.get_coach_license() now also filters is_active + expires_at,
    # but we use the full eligibility service here so the authority is consistent
    # with all other assignment flows.
    ...
    from app.repositories.tournament_repository import TournamentRepository as _TR
    _t = _TR(db).get_optional(tournament_id)
    if _t:
        _age_groups = resolve_tournament_age_groups(_t)
